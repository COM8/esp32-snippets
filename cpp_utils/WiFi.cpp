 ...
 * @return N/A.
 */
void WiFi::addDNSServer(const std::string& ip) { addDNSServer(ip.c_str()); }  // addDNSServer
 ...
 * @return N/A.
 */
void WiFi::setDNSServer(int numdns, const std::string& ip) { setDNSServer(numdns, ip.c_str()); }  // setDNSServer
 ...
 */
bool WiFi::isConnectedToAP() { return m_apConnectionStatus; }  // isConnected
...
std::vector<WiFiAPRecord> WiFi::scan() {
    ...
    std::sort(apRecords.begin(), apRecords.end(), [](const WiFiAPRecord& lhs, const WiFiAPRecord& rhs) { return lhs.m_rssi > rhs.m_rssi; });
    return apRecords;
}  // scan
 ...
 * @return N/A.
 */
void WiFi::startAP(const std::string& ssid, const std::string& password, wifi_auth_mode_t auth) { startAP(ssid, password, auth, 0, false, 4); }  // startAP
 ...
 * @return N/A.
 */
void WiFi::setIPInfo(const std::string& ip, const std::string& gw, const std::string& netmask) { setIPInfo(ip.c_str(), gw.c_str(), netmask.c_str()); }  // setIPInfo
